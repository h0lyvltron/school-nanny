...
| Option | Pros | Cons |
| --- | --- | --- |
| **Length floor only** (lean: 10–12) | Easy to explain; actually helps | Weak passwords still possible |
| **Length + HIBP k-anonymity** | Catches breached passwords without shipping the corpus | Needs egress; must **fail open** if API unreachable on private LAN |
| **Complexity theater** (upper/lower/digit/symbol) | Familiar to enterprises | Frustrates parents; weak entropy theater |

Applies to **hosted owner signup + account password change**. Does **not** apply
to family-issued PINs (separate short alphabet) or local house password.

### Confirm-email flow (sketch)

- Table `email_tokens` on `control.db`: `account_id`, `purpose=confirm`,
  `token_hash`, `expires_at`, `used_at`.
- On signup: create account, send link, set `email_verified_at` null.
- Soft vs strict before verify: lean **soft** on invite-only private deploys
  (can use app; banner to confirm); strict optional later for public VPS.
- Resend: invalidate prior confirm tokens or allow last-N; rate-limit by account.
- Unconfirmed forever: optional purge job later; not required for v1 design.

### Password reset

- Email magic link only in v1 design (no SMS).
- Same `email_tokens` with `purpose=reset`; single-use; short TTL (e.g. 1 hour).
...
- Candidates: Google / Apple / Microsoft.
- Link to **owner account** (or upgraded email adult), never to a PIN kid.
- Account linking rules: one provider subject → one account; email match must be
  verified before auto-link.
- Callback URL pain on `school-nanny.home` / LAN → **defer until public hostname**.

### Mailer adapter (interface only)
...
- Backends TBD: SMTP, provider API, and a **stdout/log** backend for local/dev.
- Template placeholders (not final copy): confirm link, reset link, optional
  “invite code used” notice to operator.
- No SMTP secrets or provider SDKs in this phase.
